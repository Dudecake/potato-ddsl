...
public:
    ...
    void confusion(vector<vector<double>> targets, vector<vector<double>> outputs) {
        ...
        size_t numClasses = targets.size(); //row= classes , col=samples
        ...
        size_t numSamples = targets[0].size();
        ...
        for (uint row = 0; row < numClasses; row++) {
            ...
            per[row][0] = static_cast<double>(a) / static_cast<double>(b);
            ...
            per[row][1] = static_cast<double>(a) / static_cast<double>(b);
            ...
            per[row][2] = static_cast<double>(a) / static_cast<double>(b);
            ...
            per[row][3] = static_cast<double>(a) / static_cast<double>(b);
    ...
    static void print(vector<double> vec, std::ostream &ss) {
        for (double d:vec) {
            ss << d << " ";
        }
    }
