	...
	namespace csv
	{
		...
		public:
			...
			void read_bom()
			{
                ...
                if (tt[0] == static_cast<char>(0xEF) || tt[1] == static_cast<char>(0xBB) || tt[2] == static_cast<char>(0xBF)) // not the correct BOM, so reset the pos to beginning (file might not have BOM)
					has_bom = true;
		...
		public:
            ...
            ofstream(const std::string& file = "", std::ios_base::openmode openMode = std::ios_base::out)
				: after_newline(true)
				, delimiter(",")
				, escape_str("##")
            ...
            ofstream(const char * file, std::ios_base::openmode openMode = std::ios_base::out)
			{
                open(file, openMode);
			}
            void open(const std::string& file, std::ios_base::openmode openMode = std::ios_base::out)
			{
				if (!file.empty())
                    open(file.c_str(), openMode);
			}
            void open(const char * file, std::ios_base::openmode openMode = std::ios_base::out)
			{
				init();
                ostm.open(file, openMode);
			}
			void init()
			{
				after_newline = true;
